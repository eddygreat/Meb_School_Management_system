...
export default function TeacherMessages() {
  ...
  const loadThreads = async () => {
    ...
    const { data } = await client.get('/comms/threads', { params: { teacher_id: user.teacher_id } })
    setThreads(data)
  }
  ...
  const createThread = async () => {
    ...
    await client.post('/comms/threads', { teacher_id: Number(user.teacher_id), parent_user_id: Number(create.parent_user_id), subject: create.subject })
    ...
    <div className="p-6 grid md:grid-cols-3 gap-4">
      <div className="space-y-3">
        ...
        <div className="bg-white p-3 rounded shadow space-y-2">
          ...
          <input className="border p-2 rounded w-full" placeholder="Parent User ID" value={create.parent_user_id} onChange={e => setCreate(v => ({ ...v, parent_user_id: e.target.value }))} />
