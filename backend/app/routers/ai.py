...
async def chat(request: ChatRequest):
    response = chat_with_student(request.message, request.context)
...
async def create_lesson_plan(request: LessonPlanRequest):
    response = generate_lesson_plan(request.topic, request.grade_level, request.subject)
    if response.startswith("Error"):
        raise HTTPException(status_code=500, detail=response)
...
async def grade_student_assignment(request: GradeRequest):
    response = grade_assignment(request.question, request.answer, request.rubric)
    if response.startswith("Error"):
        raise HTTPException(status_code=500, detail=response)
...
async def create_study_guide(request: StudyGuideRequest):
    response = generate_study_guide(request.student_data)
    if response.startswith("Error"):
        raise HTTPException(status_code=500, detail=response)
...
async def predict_student_performance(request: PredictionRequest):
    response = predict_performance(request.student_history)
    if response.startswith("Error"):
        raise HTTPException(status_code=500, detail=response)
